...
def login():
    if request.method == "POST":
        ...
        return render_template("schedule.html", course=course, subjects = subjects)
...
def schedule():
    if request.method == "POST":
        ...
        return render_template("schedule.html", subjects = subjects, course = course, due1 = due1, due2 = due2, due3 = due3, due4 = due4, due5 = due5, due6 = due6)
    else:
        course = db.execute("""SELECT subject, code, asm1, asm2, asm3, asm4, asm5, asm6 FROM assignments
        WHERE user_id=:user_id""", user_id=session["user_id"])
        ...
        idnumber = len(subject_id)

        sbj = []

        for i in range(idnumber):
            asmdue = []
            one = db.execute("""SELECT due1 FROM assignments WHERE user_id=:user_id AND subject_id=:subject_id""", user_id=session["user_id"], subject_id = subject_id[i])
            asmdue.append(one)
            two = db.execute("""SELECT due2 FROM assignments WHERE user_id=:user_id AND subject_id=:subject_id""", user_id=session["user_id"], subject_id = subject_id[i])
            asmdue.append(two)
            three = db.execute("""SELECT due3 FROM assignments WHERE user_id=:user_id AND subject_id=:subject_id""", user_id=session["user_id"], subject_id = subject_id[i])
            asmdue.append(three)
            four = db.execute("""SELECT due4 FROM assignments WHERE user_id=:user_id AND subject_id=:subject_id""", user_id=session["user_id"], subject_id = subject_id[i])
            asmdue.append(four)
            five = db.execute("""SELECT due5 FROM assignments WHERE user_id=:user_id AND subject_id=:subject_id""", user_id=session["user_id"], subject_id = subject_id[i])
            asmdue.append(five)
            six = db.execute("""SELECT due6 FROM assignments WHERE user_id=:user_id AND subject_id=:subject_id""", user_id=session["user_id"], subject_id = subject_id[i])
            asmdue.append(six)
            sbj.append(asmdue)

        return render_template("schedule.html", subjects = subjects, course = course, sbj1 = sbj[0], sbj2 = sbj[1], sbj3 = sbj[2], sbj4 = sbj[3], sbj5 = sbj[4], sbj6 = sbj[5])
