...
def login():
    if request.method == "POST":
        ...
        return render_template("schedule.html", course=course, subjects = subjects, sbj = sbj, sbjnumber = sbjnumber, subject_id = subject_id)
...
def schedule():
    if request.method == "POST":
        ...
        if move is None and remove is None:
            if asm1 is None:
                if subject is None:
                    flash("Please Enter Subject Name")
                    return redirect("/schedule")
                db.execute("""INSERT INTO subjects (user_id, subject, code, target)
                    VALUES (:user_id, :subject, :code, :target)""",
        ...
        if move is not None:
            ...
            db.execute("""UPDATE subjects SET user_id=:x, subject=:x, code=:x, target=:x, weight=:x, score=:x WHERE user_id=:user_id""", x='', user_id=session["user_id"])
            db.execute("""UPDATE assignments SET user_id=:x, subject=:x, code=:x, asm1=:x, asm2=:x, asm3=:x, asm4=:x, asm5=:x, asm6=:x,
                            due1=:x, due2=:x, due3=:x, due4=:x, due5=:x, due6=:x, weight1=:x, weight2=:x, weight3=:x, weight4=:x, weight5=:x, weight6=:x,
                            score1=:x, score2=:x, score3=:x, score4=:x, score5=:x, score6=:x WHERE user_id=:user_id""", x='', user_id=session["user_id"])

        if remove is not None:
            db.execute("""DELETE FROM subjects WHERE user_id=:user_id AND subject_id=:subject_id""", user_id=session["user_id"], subject_id=remove)
            db.execute("""DELETE FROM assignments WHERE user_id=:user_id AND subject_id=:subject_id""", user_id=session["user_id"], subject_id=remove)
        ...
        return render_template("schedule.html", subjects = subjects, course = course, sbj = sbj, sbjnumber = sbjnumber, subject_id = subject_id)
    else:
        course = db.execute("""SELECT subject, code, asm1, asm2, asm3, asm4, asm5, asm6 FROM assignments
        WHERE user_id=:user_id""", user_id=session["user_id"])
...
def subject():
    if request.method == "POST":
        ...
        for i in range(sbjnumber):
            j = []
            subscore = db.execute("""SELECT score1, score2, score3, score4, score5, score6 FROM assignments WHERE user_id=:user_id AND subject_id=:subject_id""", user_id=session["user_id"], subject_id=subject_id[i]["subject_id"])
...
def record():
    if request.method == "POST":
        ...
        for i in range(len(semester)):
            subjects = db.execute("""SELECT subject FROM records WHERE user_id=:user_id AND semester=:semester""", user_id=session["user_id"], semester=semester[i]["semester"])
            sub = []
            for j in range(len(subjects)):
                sub.append(subjects[j]["subject"])
            subject.append(sub)

            subject_id = db.execute("""SELECT subject_id FROM records WHERE user_id=:user_id AND semester=:semester""", user_id=session["user_id"], semester=semester[i]["semester"])
            sub_id = []
            for j in range(len(subject_id)):
                sub_id.append(subject_id[j]["subject_id"])

            codes = db.execute("""SELECT code FROM records WHERE user_id=:user_id AND semester=:semester""", user_id=session["user_id"], semester=semester[i]["semester"])
            sub = []
            for j in range(len(codes)):
                sub.append(codes[j]["code"])
            code.append(sub)

            scores = db.execute("""SELECT score FROM records WHERE user_id=:user_id AND semester=:semester""", user_id=session["user_id"], semester=semester[i]["semester"])
            sub = []
            for j in range(len(scores)):
                sub.append(scores[j]["score"])
            score.append(sub)

            targets = db.execute("""SELECT target FROM records WHERE user_id=:user_id AND semester=:semester""", user_id=session["user_id"], semester=semester[i]["semester"])
            sub = []
            for j in range(len(targets)):
                sub.append(targets[j]["target"])
            target.append(sub)

            asms = []
            for k in range(len(sub_id)):
                sub = []
                asm1 = db.execute("""SELECT asm1 FROM records WHERE user_id=:user_id AND semester=:semester AND subject_id=:subject_id""",
                                    user_id=session["user_id"], semester=semester[i]["semester"], subject_id=sub_id[k])
                sub.append(asm1[0]['asm1'])
                asm2 = db.execute("""SELECT asm2 FROM records WHERE user_id=:user_id AND semester=:semester AND subject_id=:subject_id""",
                                    user_id=session["user_id"], semester=semester[i]["semester"], subject_id=sub_id[k])
                sub.append(asm2[0]['asm2'])
                asm3 = db.execute("""SELECT asm3 FROM records WHERE user_id=:user_id AND semester=:semester AND subject_id=:subject_id""",
                                    user_id=session["user_id"], semester=semester[i]["semester"], subject_id=sub_id[k])
                sub.append(asm3[0]['asm3'])
                asm4 = db.execute("""SELECT asm4 FROM records WHERE user_id=:user_id AND semester=:semester AND subject_id=:subject_id""",
                                    user_id=session["user_id"], semester=semester[i]["semester"], subject_id=sub_id[k])
                sub.append(asm4[0]['asm4'])
                asm5 = db.execute("""SELECT asm5 FROM records WHERE user_id=:user_id AND semester=:semester AND subject_id=:subject_id""",
                                    user_id=session["user_id"], semester=semester[i]["semester"], subject_id=sub_id[k])
                sub.append(asm5[0]['asm5'])
                asm6 = db.execute("""SELECT asm6 FROM records WHERE user_id=:user_id AND semester=:semester AND subject_id=:subject_id""",
                                    user_id=session["user_id"], semester=semester[i]["semester"], subject_id=sub_id[k])
                sub.append(asm6[0]['asm6'])
                asms.append(sub)
            asm.append(asms)
            ...
            for k in range(len(sub_id)):
                sub = []
                score1 = db.execute("""SELECT score1 FROM records WHERE user_id=:user_id AND semester=:semester AND subject_id=:subject_id""",
                                    user_id=session["user_id"], semester=semester[i]["semester"], subject_id=sub_id[k])
                sub.append(score1[0]['score1'])
                score2 = db.execute("""SELECT score2 FROM records WHERE user_id=:user_id AND semester=:semester AND subject_id=:subject_id""",
                                    user_id=session["user_id"], semester=semester[i]["semester"], subject_id=sub_id[k])
                sub.append(score2[0]['score2'])
                score3 = db.execute("""SELECT score3 FROM records WHERE user_id=:user_id AND semester=:semester AND subject_id=:subject_id""",
                                    user_id=session["user_id"], semester=semester[i]["semester"], subject_id=sub_id[k])
                sub.append(score3[0]['score3'])
                score4 = db.execute("""SELECT score4 FROM records WHERE user_id=:user_id AND semester=:semester AND subject_id=:subject_id""",
                                    user_id=session["user_id"], semester=semester[i]["semester"], subject_id=sub_id[k])
                sub.append(score4[0]['score4'])
                score5 = db.execute("""SELECT score5 FROM records WHERE user_id=:user_id AND semester=:semester AND subject_id=:subject_id""",
                                    user_id=session["user_id"], semester=semester[i]["semester"], subject_id=sub_id[k])
                sub.append(score5[0]['score5'])
                score6 = db.execute("""SELECT score6 FROM records WHERE user_id=:user_id AND semester=:semester AND subject_id=:subject_id""",
                                    user_id=session["user_id"], semester=semester[i]["semester"], subject_id=sub_id[k])
                sub.append(score6[0]['score6'])
                subscores.append(sub)
            subscore.append(subscores)

        return render_template("record.html", semester=semester, subject=subject, code=code, score=score, target=target, asm=asm, subscore=subscore)
