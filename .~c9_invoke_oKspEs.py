...
def subject():
    if request.method == "POST":
        ...
        if score1 is None or weight1 is None:
            score1 = 0
            weight1 = 0
        if score2 is None or weight2 is None:
            score2 = 0
            weight2 = 0
        if score3 is None or weight3 is None:
            score3 = 0
            weight3 = 0
        if score4 is None or weight4 is None:
            score4 = 0
            weight4 = 0
        if score5 is None or weight5 is None:
            score5 = 0
            weight5 = 0
        if score6 is None or weight6 is None:
            score6 = 0
            weight6 = 0
        score = (score1 * weight1 + score2 * weight2 + score3 * weight3 + score4 * weight4 + score5 * weight5 + score6 * weight6) * 0.01
        target = request.form.get(target)
        ...
        if target is None:
            ...
            db.execute("""UPDATE subject SET score=:score WHERE user_id=:user_id AND subject=:subject""", score=score, user_id=session["user_id"], subject=subject)
        else:
            db.execute("""UPDATE subject SET target=:target WHERE user_id=:user_id AND subject=:subject""", target=target, user_id=session["user_id"], subject=subject)
        ...
        score = db.execute("""SELECT score FROM subject WHERE user_id=:user_id""", user_id=session["user_id"])
        scores = []
        for i in range(len(score)):
            scores.append(score[i]['score'])

        target = db.execute("""SELECT target FROM subject WHERE user_id=:user_id""", user_id=session["user_id"])
        targets = []
